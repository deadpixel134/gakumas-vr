...
- 신규 설정과 설정 GUI의 기본값 복원에서 live 독립 6DoF를 기본 활성화했다. 라이브 진입 anchor가 연출 카메라 경로·각도와 독립적인 기존 v0.173 동작은 유지한다.
- 스냅 턴 기본 각도를 15°에서 30°로 변경했다. 선택 가능한 15°/30°/45°/60°와 smooth 모드, 우세축·재무장 상태기는 유지한다.
- 위치 이동 속도 기본값을 1.50m/s에서 정확히 1.3배인 1.95m/s로 변경했다. final-view 완전 3D 이동과 physical HMD roll 격리 방식은 바꾸지 않았다.
- 기존 `settings.json`은 업데이트에서 덮어쓰지 않는다. 위 값은 신규 설정과 기본값 복원에 적용되며, 현재 로컬 실사용 설정의 이동 속도는 별도로 1.95m/s로 맞췄다.
- 런타임 빌드·패키지·설치 SHA-256은 `4D93D785B5B0E4318ACB8EADA50D61A1D2D77B9463B64C0FACFF3FC9D68B6E15`, 배포 ZIP SHA-256은 `5064E59894BBF46996436AEE9DDF222201E66317A808166CA527A41B00677CCD`다. v0.173 설치 상태는 `rollback/product-install-0.174.0-20260813-224142/`에 중첩 보관했다.
