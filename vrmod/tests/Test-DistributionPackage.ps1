...
$installScript = Join-Path $package 'Install-GakumasVR.ps1'
$uninstallScript = Join-Path $package 'Uninstall-GakumasVR.ps1'
...
Assert-True ($doorstopConfig -match '(?m)^target_assembly\s*=\s*vrmod\\runtime\\GakumasVR\.RuntimeBootstrap\.dll\s*$') 'Doorstop target assembly is invalid.'
...
$temporaryBase = [System.IO.Path]::GetFullPath(
    (Join-Path ([System.IO.Path]::GetTempPath()) 'GakumasVR.Distribution.Tests'))
...
try {
    ...
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $cleanGame 'gakumas-local'))) 'Clean install created gakumas-local.'
    ...
    [System.IO.Directory]::CreateDirectory((Join-Path $localifyGame 'gakumas-local')) | Out-Null
    [System.IO.File]::WriteAllText((Join-Path $localifyGame 'gakumas-local\config.json'), '{}')
