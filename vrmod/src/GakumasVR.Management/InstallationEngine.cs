...
public sealed class InstallationEngine
{
    ...
    public static LocalifyStatus DetectLocalify(string gameRoot)
    {
        ...
        string localifyRoot = Path.Combine(gameRoot, "gakumas-local");
        int markerCount = new[] { "config.json", "localizationConfig.json", "version.txt" }
            .Count(file => File.Exists(Path.Combine(localifyRoot, file)));
        if (proxy && markerCount > 0)
        {
            return LocalifyStatus.Installed;
        }
        if (proxy || markerCount > 0 || Directory.Exists(localifyRoot))
        {
    ...
    public static bool IsGameRoot(string path) =>
        File.Exists(Path.Combine(path, "gakumas.exe")) &&
    ...
    private static string ResolveContainedPath(string root, string relativePath)
    {
        ...
        if (string.Equals(normalized, "version.dll", StringComparison.OrdinalIgnoreCase) ||
            string.Equals(normalized, "gakumas-local", StringComparison.OrdinalIgnoreCase) ||
            normalized.StartsWith("gakumas-local" + Path.DirectorySeparatorChar, StringComparison.OrdinalIgnoreCase))
        {
