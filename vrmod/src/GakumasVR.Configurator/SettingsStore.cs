...
internal static class SettingsStore
{
    ...
    public static string FindInitialGameRoot()
    {
        ...
        {
            if (File.Exists(Path.Combine(current.FullName, "gakumas.exe")))
            {
    ...
    {
        EnsureGameStopped();
        if (!File.Exists(Path.Combine(gameRoot, "gakumas.exe")))
        {
    ...
    private static void EnsureGameStopped()
    {
        ...
        if (Process.GetProcessesByName("gakumas").Any(process =>
