...
現在の公開版は **v0.163.0プレリリース**です。Meta Quest 2＋Virtual Desktop OpenXRで開発・実機確認しています。SteamVR OpenXRとMeta Quest Linkは予備対応であり、本プロジェクトではまだ実機確認していません。
...
- v0.163.0以降の配布ZIPには必要なDobbyが含まれます。既存のLocalify／BepInEx環境が同じパスのファイルを提供している場合、インストーラーは上書きせず保持します。
