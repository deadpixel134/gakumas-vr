...
現在の公開版は **v0.162.0プレリリース**です。Meta Quest 2＋Virtual Desktop OpenXRで開発・実機確認しています。SteamVR OpenXRとMeta Quest Linkは予備対応であり、本プロジェクトではまだ実機確認していません。
...
- 右手レイポインター、A／トリガークリック、Bで戻る、スティックスクロール
...
- Windows 11 x64、DMM版、Unity 6000.0.77f1を対象とするプレリリースです。
- v0.162.0ランタイムは`BepInEx/core/dobby.dll`に依存しますが、現在の配布ZIPにはこのファイルが含まれていません。互換性のあるLocalify／BepInEx環境がない完全なクリーンインストールは、まだ対応・検証されていません。
