...
VDXR (Virtual Desktop)을 이용해 플레이할 수 있으며, 이 경로를 이용한 실기 테스트도 완료했습니다. 현재 공개 버전은 **v0.175.6 정식 릴리스**입니다. 핵심 6DoF 조작은 v0.173에서 실기 검증되었고, v0.175.6는 그 동작을 유지하면서 공간/스케일 프로필, 설정 GUI, 설치 검증과 배포 기준을 정리한 현재 공식 버전입니다. SteamVR OpenXR와 Meta Quest Link는 예비 지원 대상으로 남아 있으며, 아직 이 프로젝트에서 실기 검증하지 않았습니다.
...
- [재사용 가능한 VR 조작·포즈 합성 명세](docs/ko/VR_INTERACTION_SPEC.md)
...
- 오른손 3D 이동, 왼손 월드축 시야 회전과 30° 기본 스냅 턴
- 오른손 레이 포인터, A/트리거 클릭, B 뒤로 가기
...
- Windows 11 x64, DMM판, Unity 6000.0.77f1 기준입니다.
- 필요한 Dobby를 배포 ZIP에 포함합니다. v0.175.6 설치기는 게임 파일을 건드리기 전에 전체 payload와 클린 설치 필수 구성의 해시·보존 정책을 사전 검증합니다.
