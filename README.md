...
현재 공개 버전은 **v0.162.0 프리릴리스**입니다. Meta Quest 2 + Virtual Desktop OpenXR에서 개발·검증했습니다. SteamVR OpenXR와 Meta Quest Link는 규격상 사용 가능한 예비 지원 대상으로, 아직 이 프로젝트에서 실기 검증하지 않았습니다.
...
- v0.162.0 런타임은 `BepInEx/core/dobby.dll`에 의존하지만 현재 배포 ZIP은 이 파일을 포함하지 않습니다. 기존 Localify/BepInEx 환경에 호환 파일이 없는 완전한 클린 설치는 아직 지원·검증되지 않았습니다.
