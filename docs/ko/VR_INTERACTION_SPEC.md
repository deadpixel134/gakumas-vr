...
이 문서는 Gakumas VR v0.173.0에서 사용자 실기로 승인된 조작감과 안전 구조를 다른 Unity 게임의 VR 모드에서도 재현하기 위한 구현 계약이다. 게임별 카메라 탐색 코드는 달라질 수 있지만, 아래 좌표·입력·표시 불변식은 그대로 유지해야 같은 동작을 기대할 수 있다.
...
- 기본 회전은 15° 스냅이며 15°/30°/45°/60°를 선택할 수 있다. 부드러운 연속 회전도 선택할 수 있다.
...
- deadzone 기본값은 0.20, 이동 속도 기본값은 1.5m/s다.
