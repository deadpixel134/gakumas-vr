...
최종 갱신: 2026-08-15
현재 마일스톤: **M8 — 런타임 호환성·유지보수**
현재 소스/설치본 v0.175.5 로컬 후보, M7 사용자 실기 기준 v0.173 유지, M8 공간/크기 후보의 GUI·VR 실기 진행 전
...
| M7 | **달성 — 2026-08-13** | 입력·설정·설치 제품화와 6DoF 조작 기준 | 사용자 승인 ray/A·Trigger/B/Grip과 정면·손 패널을 유지하고, 다국어 설정 schema/GUI, 설치·제거·rollback, GitHub 자동 업데이트, 비-live 6DoF·선택형 live 6DoF, 손 교체형 이동/월드축 회전과 실제 HMD roll만 보존하는 조작을 사용자 실기로 확정 |
| M8 | **진행 중 — 런타임 호환성·유지보수** | 배포 기본값·런타임 호환성·유지보수 | v0.174 최신 정식 Release와 v0.173 사용자 실기 기준을 구분해 유지하고, v0.175.5 공간/크기 후보의 GUI·live/non-live VR 회귀를 확인하며 SteamVR/Quest Link 예비 경로, 게임 업데이트 호환성 gate와 개인정보 없는 진단 흐름을 확장 |

## M8 현재 근거와 미완료 조건

- v0.175.5는 코어 43/43, 관리 7/7, Release 빌드, 199개 manifest, clean install, Localify 공존·제거 및 실제 설치 해시 확인을 마쳤다.
- 그러나 공간/크기 탭의 최종 실제 GUI 배치, 100% 자동 회귀, live/non-live별 다른 크기, 눈 간격·머리 이동·스틱 이동 수동 override, 평면 패널/입력/폴백 회귀는 사용자 VR 실기로 아직 확인하지 않았다.
- 따라서 이 자동 검증과 설치는 M8 달성 근거가 아니며, M8 상태는 계속 진행 중이다.
...
M7의 최종 범위는 사용자 결정에 따라 직접 터치와 live 좌상단 시계 회귀를 제외하고, 설정·설치·업데이트 제품화와 범용 6DoF 조작을 기준으로 판정했다. 두 제외 항목은 구현 또는 검증 완료로 간주하지 않는다.

1. v0.155~v0.160에서 버전 있는 JSON 설정 schema, 한국어 기본의 한·영·일 설정 GUI, 설정 가져오기/내보내기/기본값 복원, manifest 기반 설치·제거·rollback과 독립 EXE 설치기를 완성했다.
2. v0.161~v0.164에서 자동/수동 VFX, 0.50~2.00 eye render scale과 고배율 경고, Dobby 포함 배포 및 payload 전체 사전 검증·clean install·Localify 공존 검사를 확정했다.
3. v0.165~v0.166에서 event/cache 기반 3D 전환 fast path와 GitHub stable Release 기반 자동 업데이트를 추가했다. 업데이트는 게임 실행 중 교체하지 않고 ZIP과 SHA-256이 일치할 때만 설치 흐름으로 넘긴다.
4. v0.167~v0.170에서 비-live positional 6DoF, 설정 선택형 live 독립 6DoF, 좌우 교환 가능한 final-view 완전 3D 이동과 반대 손 시야 회전을 구현했다. VR 환경의 thumbstick scroll은 비활성화했다.
5. v0.171~v0.173에서 우세축 기반 월드 yaw/pitch 스냅(기본 15°, 선택 15/30/45/60°)·부드러운 회전과 HMD 성분별 원점 비교를 적용했다. scene/스틱/진입 자세의 roll은 버리고 사용자가 실제로 HMD를 기울인 roll 변화량만 마지막에 합성한다.
6. 코어 39/39, 관리 7/7, Release 빌드, 199개 manifest와 네이티브 의존성, clean install 및 Localify 공존·제거 검사를 통과했다. 런타임 SHA-256은 `0851E03A8A30B3FB3822C4626120CFB7463CC8C81405067D8A6659D36234FD15`, 배포 ZIP SHA-256은 `CFC89565B303C631E0A4694774D40D4F74D19C8FC6B80E7F557E772FF121C9B0`다.
7. 사용자는 v0.173 VR 실기 후 결과를 “완벽했다”고 최종 판정했다. 따라서 M7을 2026-08-13 달성했다.

상세 수학·상태기·수명·폴백과 타 게임 이식 경계는 [`ko/VR_INTERACTION_SPEC.md`](ko/VR_INTERACTION_SPEC.md)를 따른다. 다음 단계 M8은 SteamVR/Quest Link 실기, 게임 업데이트 호환성 gate와 정식 배포 유지보수다.

## M8 진행 기록 — v0.174 기본 프로필

1. 사용자의 명시적 요청에 따라 신규 설정과 기본값 복원의 live 독립 6DoF를 ON으로 변경했다.
2. 기본 스냅 턴을 15°에서 30°로, 기본 이동 속도를 1.50m/s에서 1.95m/s로 변경했다. v0.173에서 승인된 이동·회전·roll 합성 수학은 바꾸지 않았다.
3. 코어 39/39, 관리 7/7, Release 빌드, 199개 manifest, 네이티브 의존성, clean install, Localify 공존·제거와 실제 설치 해시 일치를 확인했다.
4. v0.174 사용자 VR 실기는 아직 진행하지 않았으므로 M8 달성이나 v0.174 실기 완료로 판정하지 않는다. 기존 `settings.json` 보존은 업데이트 안전 계약으로 유지한다.
